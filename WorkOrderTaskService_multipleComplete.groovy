...
class WorkOrderTaskService_Multiplecomplete extends ServiceHook{
	...
	public Object onPreExecute(Object dto) {
		...
		ArrayList<WorkOrderTaskServiceCompleteRequestDTO>  c = (ArrayList<WorkOrderTaskServiceCompleteRequestDTO>) dto 
		...
		for (int i = 0; i < c .size(); i++) {
				...
				task_nmr = c[i].getWOTaskNo().toString()
				dis_code = c[i].getDistrictCode().toString()
				work_order = c[i].getWorkOrder().toString()
